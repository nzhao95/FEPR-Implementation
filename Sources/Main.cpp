...
struct Light {
    ...
    void setupCameraForShadowMapping(glm::vec3 scene_center , float scene_radius) {
      glm::mat4 M = glm::ortho(-scene_radius,scene_radius,-scene_radius,scene_radius,0.1f,10.f);
      glm::mat4 V = glm::lookAt(m_position, scene_center, glm::vec3(0.0,-1.0,0.0));
...
struct Scene{
    ...
    void render() {
        ...
        // Set the model 0 : Phong;, 1 : Microfacet, 2 : Toon

        shaderProgramPtr->set (std::string("model"), model);
        ...
        {
            for (int k = 0; k < scene_lights.size(); k++) {
              Light & light = scene_lights[k];
              shaderProgramPtr->set (std::string("lightSources["+std::to_string(k)+"].position"), light.m_position);
              shaderProgramPtr->set (std::string("lightSources["+std::to_string(k)+"].color"), light.m_color);
              shaderProgramPtr->set (std::string("lightSources["+std::to_string(k)+"].intensity"), light.m_intensity);
              shaderProgramPtr->set (std::string("lightSources["+std::to_string(k)+"].isActive"), 1);
              shaderProgramPtr->set (std::string("lightSources["+std::to_string(k)+"].depthMVP"), light.depthMVP);
              shaderProgramPtr->set (std::string("lightShadowMaps["+std::to_string(k)+"]"), (int) light.shadowMapTexOnGPU);
            }
        }

        //Set Texture
        // shaderProgramPtr->set (std::string("albedoTex"), (int) albedoTexOnGPU);


        // Set the material for the center mesh :
        shaderProgramPtr->set ("material.albedo", glm::vec3( 0.9 , 0.7 , 0.2 ) );
        shaderProgramPtr->set ("material.shininess" , (float) 50.f );
        shaderProgramPtr->set ("material.F0" , glm::vec3(1.0, 0.86, 0.57) );
        shaderProgramPtr->set ("material.roughness" , (float) 1.0 );
        shaderProgramPtr->set ("material.metalness" , (float) 1.f );
...
int filter_w = 0;
bool filter_normal = false;
glm::vec3 filter_scale (0.6,0.6,1.0);
...
void initScene (const std::string & meshFilename) {
    ...
    // Setup lights :
    {
...

        {
            ...
            newLight.m_position = glm::vec3 (-2*cos(app_timer), 2*sin(app_timer), 2.0);
            newLight.m_color = glm::vec3(1.0, 0.5, 0.5);
            newLight.m_intensity = 0.4f;
        ...
        }
        {
            scene.scene_lights.resize( scene.scene_lights.size() + 1 );
            Light & newLight = scene.scene_lights[ scene.scene_lights.size() - 1 ];
            newLight.m_position = glm::vec3 (5.0, -2, 5.0);
            newLight.m_color = glm::vec3(0.5, 0.5, 1.0);
        ...
        }
        {
            scene.scene_lights.resize( scene.scene_lights.size() + 1 );
            Light & newLight = scene.scene_lights[ scene.scene_lights.size() - 1 ];
            newLight.m_position = glm::vec3 (0.0, 0.0, -5.0);
            newLight.m_color = glm::vec3(1.0, 1.0, 1.0);
            newLight.m_intensity = 0.6f;
            newLight.allocateShadowMapFBO(shadow_map_width , shadow_map_height);
            newLight.shadowMapTexOnGPU = texture_slot_available;   ++texture_slot_available;
        }
    ...
    {
      albedoTexOnGPU = texture_slot_available; texture_slot_available++;
    ...
    // Adjust the camera to the actual mesh
    {
        ...
        scene.scene_center = meshCenter;
        scene.scene_radius = meshRadius;
